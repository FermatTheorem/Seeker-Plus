...
class Hackertarget(Engine):
    ...
    def reverseiplookup(self, url):
        self.log_info("Enumerating subdomains with Hackertarget reverse IP lookup...")
        IP = self.get_ip(url)
        if IP:
            ...
            if response.status_code == 200:
                if 'API count exceeded' in response.text:
                    self.log_info("API count exceeded\n")
                    return
    ...
    def hostsearch(self, url):
        self.log_info("Enumerating subdomains with Hackertarget hostsearch API...")
