import requests, os, fake_useragent, logging, threading, socket
...
class SuperClass():
    ...
    def validateUrls(self, urls):
        ...
        res = [url for future, url in zip(futures, urls) if future.result()]
        print(f"{len(res)} of {len(urls)} urls seem to be valid")
        return res
